...
public class ValidateCodeUtil {
    ...
    private static final String sessionKey = "LOGINCHECKCODE";
    ...
    public static void getRandCodeImageAndSaveRedis(String prefix, String idNumber,
                                                    HttpServletResponse response, StringRedisTemplate stringRedisTemplate, Long time, TimeUnit unit){
        ...
        stringRedisTemplate.opsForValue().set(prefix+idNumber, randomStr, time, unit);
    ...
    //生成随机图片
    public static void getRandomCodeImage(HttpServletRequest request, HttpServletResponse response){
...
//        System.out.println("随机字符："+randomStr);
    ...
    //生成随机图片的base64编码字符串
    public static String getRandomCodeBase64(HttpServletRequest request, HttpServletResponse response) {
        HttpSession session = request.getSession();
        // BufferedImage类是具有缓冲区的Image类,Image类是用于描述图像信息的类
        BufferedImage image = new BufferedImage(width, height, BufferedImage.TYPE_INT_BGR);
        Graphics g = image.getGraphics();
